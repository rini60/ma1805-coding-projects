...
rotateX(70) ; 
  ...
  beginShape(POINTS)
  for(theta = 0; theta; theta < 60; theta += 1){ 
    for (let  phi = 0; phi < 360 phi += 2) {  
    let r = (70 * pow(abs(sin(phi * *8/2)), 1) + 250) * theta/60; //multiply phi with arbitrary value, move sin wave up to create my prefered shape, added powers to edit the sharpness of the petals, makes shape more dense and flat when multipying the radius by theta/60
    let x = r * cos (phi); 
    let y = r * sin(phi); 
